            ...
            <div class="posts-grid">
              ...
              >
                ...
                <a-form-model-item class="form-item mb-3" label="Описание">
                  <quill-editor
                    v-model="form.feedback[item.index]"
...
export default {
  ...
  mounted() {
    this.__GET_POSTS_BY_ID(this.$route.params.id);
  },
  methods: {
    onSubmit() {
      this.$refs["ruleForm"].validate((valid) => {
        if (valid) {
          this.__EDIT_FEEDBACKS(this.form);
        } else {
          return false;
        }
      });
    ...
    async __GET_POSTS_BY_ID(id) {
      try {
        this.spinning = true;
        const data = await this.$store.dispatch("fetchFeedbacks/getFeedbacksById", id);
        this.form = { ...data.feedback };
        if (this.form.logo) {
          this.fileList = [
            {
              uid: "-1",
    ...
    handleChange({ fileList }) {
      ...
      if (fileList[0]?.response?.path) {
        this.form.logo = fileList[0]?.response?.path;
